...
/* OrderProcessServiceController: controller class */
public class OrderProcessServiceController {
	...
	public OrderProcessServiceController() {
	}

	/**
	 * This method creates an account if the user has not signed up already
	 * 
	 * @param requestBody
	 * @return Response
	 */
	...
	public Response confirmOrder(String requestBody) {
		try {
			...
			if ((checkTransactionsCount > 1) && (checkTransactionsCount % 5 == 0)) {
				String status = com.ecom.orderProcessService.exceptions.Status.DENIED.getDescription();
				orderRequest.setStatus(status);
				orderProcessService.placeOrder(orderRequest);
				return Response.status(Status.FORBIDDEN).type(MediaType.APPLICATION_JSON)
						.entity(OrderProcessServiceUtil.responseString(OrderProcessServiceConstant.ORDER_DENIED))
						.build();
			}
			orderProcessService.placeOrder(orderRequest);
